...
API_URL = "http://endpoint-raman:8001/predict"
...
def predict_udf(data):
    ...
    return response[0]["Predict_Pen"] if response else 0.0

def write_to_postgres(batch_df, batch_id):
    batch_df.write \
        .format("jdbc") \
        .option("url", jdbc_url) \
        .option("dbtable", "raman_db") \
...
def analyze(sc):
    ...
    data = data.fillna(0)
    data = data.withColumn("prediction", predict_udf(struct(feature_cols[5:])))
    feature_cols = [str(col) for col in range(1300,1050,-1)]
    feature_cols.insert(0, "Time stream") 
    feature_cols.insert(1, "Cust") 
    feature_cols.insert(2, "Project_ID") 
    feature_cols.insert(3, "BatchID") 
    feature_cols.insert(4, "Scan") 
    feature_cols.append("prediction")
    ...
    # query = data.writeStream.format("console").start()
